...
public class AlgorithmTester {
	...
	public void execute(GlobalIndex index, String outFolder)
			throws JATEException, IOException {
		...
		/* .extractNP(c); */
		for (Map.Entry<Algorithm, AbstractFeatureWrapper> en : _algregistry
				.entrySet()) {
			_logger.info("Running feature store builder and ATR..."
					+ en.getKey().toString());
			result = en.getKey().execute(en.getValue());
			writer.output(result, outFolder + File.separator
					+ en.getKey().toString() + ".txt");
		}

	}

	public void execute(GlobalIndex index, Context context)
			throws JATEException, IOException, InterruptedException {
		// NCValue code modification begins
	...
	public void execute(Context context) throws JATEException, IOException,
	...
	public static void main(String[] args) {
		if (args.length < 3)
			System.out
					.println("Usage: java AlgorithmTester [corpus_path] [reference_corpus_path] [output_folder]");
		else {
			try {
				...
				// "D:/work/JATR_SDK/jate_googlecode/test/output/worddb");
				// GlobalIndexWriterHSQL.persist(termDocIndex,
				// "D:/work/JATR_SDK/jate_googlecode/test/output/termdb");
				...
				// in corpora/documents
				WordCounter wordcounter = new WordCounter();

				// Next we need to count frequencies of candidate terms. This is
				// a computational extensive process
				// and can take long for large corpus.
				//
				// There are two ways of doing this, the first is to use
				// multi-thread
				// counting process, by which the corpus is split into sections
				// and several threads are run in parallel
				// for counting; the second is a single thread option. The first
				// will use more memory and CPU but faster
				// on large corpus.

				/*
				 * #1 Due to use of multi-threading, this can significantly
				 * occupy your CPU and memory resources. It is better to use
				 * this way on dedicated server machines, and only for very
				 * large corpus
				 */
				FeatureCorpusTermFrequency wordFreq = new FeatureBuilderCorpusTermFrequencyMultiThread(
						wordcounter, lemmatizer).build(wordDocIndex);
				FeatureDocumentTermFrequency termDocFreq = new FeatureBuilderDocumentTermFrequencyMultiThread(
						wordcounter, lemmatizer).build(termDocIndex);
				FeatureTermNest termNest = new FeatureBuilderTermNestMultiThread()
						.build(termDocIndex);
				FeatureRefCorpusTermFrequency bncRef = new FeatureBuilderRefCorpusTermFrequency(
						args[1]).build(null);
				FeatureCorpusTermFrequency termCorpusFreq = new FeatureBuilderCorpusTermFrequencyMultiThread(
						wordcounter, lemmatizer).build(termDocIndex);
				 ...
				 * termDocFreq = new
				 * FeatureBuilderDocumentTermFrequency(npcounter, wordcounter,
				...
				// tester.registerAlgorithm(new GlossExAlgorithm(), new
				// GlossExFeatureWrapper(termCorpusFreq, wordFreq, bncRef));
				// tester.registerAlgorithm(new WeirdnessAlgorithm(), new
				// WeirdnessFeatureWrapper(wordFreq, termCorpusFreq, bncRef));
				tester.registerAlgorithm(new CValueAlgorithm(),
						new CValueFeatureWrapper(termCorpusFreq, termNest));
				// tester.registerAlgorithm(new TermExAlgorithm(), new
				// TermExFeatureWrapper(termDocFreq, wordFreq, bncRef));
				tester.registerAlgorithm(new RIDFAlgorithm(),
						new RIDFFeatureWrapper(termCorpusFreq));
